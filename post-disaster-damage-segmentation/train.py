...
if __name__ == "__main__":
    ...
    parser.add_argument('--attention', type=str, default=None,
                       choices=['none', 'cbam', 'eca', 'esam'],
                       help='注意力模块类型 (仅对 enhanced_building_damage 有效)')
    parser.add_argument('--local_rank', '--local-rank', type=int, default=-1,
                       help='DDP 分布式训练的 local rank（自动设置）')
    ...
    #   针对增强版模型的优化建议:
    #   - light级别: 保持原有参数
    #   - moderate级别: 可适当降低学习率 (Init_lr *= 0.8)
    #   - strong级别: 建议降低学习率 (Init_lr *= 0.6) 并增加训练轮数
    #------------------------------------------------------------------#
    optimizer_type      = "sgd"
    momentum            = 0.9
    weight_decay        = 5e-4
    ...
    #------------------------------------------------------------------#
    VOCdevkit_path  = r'/home/luminous/Hxd/Disaster/Code/Disaster_seg/deeplab/MAG_deeplab_test1/VOCdevkit_old'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    cls_weights = np.array([
        0.25,  # background - 降低权重，因为背景容易分类
        1.4,  # undamaged_building - 基础权重
        1.1,  # slight_damage - 稍微提高
        1.4,  # moderate_damage - 更高权重
        1.1   # severe_damage - 最高权重，因为样本最少但最重要
    ], np.float32)
    #------------------------------------------------------------------#
    ...
    if distributed:
        ...
        # 优先从环境变量获取 LOCAL_RANK，如果没有则使用 0
    ...
    #----------------------------------------------------#
    #   模型选择: 原版DeepLabV3+ 或 增强版建筑损伤模型
    #   model_type: "original", "enhanced_building_damage"
    #----------------------------------------------------#
    model_type = "enhanced_building_damage"  # 推荐: "enhanced_building_damage", 可选: "original"
    
    #----------------------------------------------------#
    #   注意力模块选择（仅对 enhanced_building_damage 有效）
    ...
    #   
    #   分布式训练：
    #   CUDA_VISIBLE_DEVICES=0,1,2 python -m torch.distributed.launch --nproc_per_node=3 train.py --attention esam
    #----------------------------------------------------#
    attention_type = args.attention if args.attention is not None else 'esam'  # 默认使用ESAM
    
    # 初始化损失函数标志
    use_enhanced_loss = False
    enhanced_loss_fn = None
    
    if model_type == "original":
        # 使用原版DeepLabV3+
        from nets.deeplabv3_plus import DeepLab
        model = DeepLab(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor, pretrained=pretrained)
        print(f"🔧 使用原版DeepLabV3+模型")
        ...
        from nets.enhanced_loss import EnhancedDamageLoss
        ...
        # 初始化增强损失函数
        enhanced_loss_fn = EnhancedDamageLoss(num_classes=num_classes)
        use_enhanced_loss = True
        
        print(f"🚀 使用增强版建筑损伤DeepLabV3+模型")
        print(f"🔧 注意力模块: {attention_type.upper()}")
        ...
        print(f"📝 描述: {attention_desc.get(attention_type, '未知')}")
        
        print(f"🎯 特性: 7分支损伤ASPP + 梯度感知 + 增强损失")
        print(f"📊 目标: 解决slight/moderate损伤混淆问题")
    else:
        raise ValueError(f"不支持的模型类型: {model_type}，请选择 'original' 或 'enhanced_building_damage'")
    model.to('cpu')  # 保证模型和所有子模块都在CPU上，避免summary报错
    
    # 显示模型结构 (如果 torchsummary 可用)
    if HAS_TORCHSUMMARY:
        with torch.no_grad():
            try:
                summary(model, input_size=(3, input_shape[0], input_shape[1]), device='cpu')
            except Exception as e:
                print(f"Warning: Could not display model summary: {e}")
    else:
        print("Skipping model summary (torchsummary not available)")
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if True:
        ...
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir, Cuda, \
                                            ...
                                            train_image_ids=None)  # 传入训练集用于过拟合检测
        else:
            eval_callback   = None
        #---------------------------------------#
        #   开始模型训练
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank, use_enhanced_loss, enhanced_loss_fn)
