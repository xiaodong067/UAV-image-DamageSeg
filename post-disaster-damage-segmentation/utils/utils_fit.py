...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, \
    fp16, scaler, save_period, save_dir, local_rank=0, use_enhanced_loss=False, enhanced_loss_fn=None):
    total_loss      = 0
    total_f_score   = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #   计算损失
            #----------------------#
            if use_enhanced_loss and enhanced_loss_fn is not None:
                # 使用增强损失函数
                loss, loss_details = enhanced_loss_fn(outputs, pngs)
            else:
                # 使用原始损失函数
                if focal_loss:
